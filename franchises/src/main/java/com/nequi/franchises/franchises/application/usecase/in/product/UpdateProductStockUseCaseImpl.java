...
public class UpdateProductStockUseCaseImpl implements UpdateProductStockUseCase {
    ...
    @Override
    public Mono<ProductResponse> execute(String franchiseId, UpdateProductStockRequest request) {
        log.info("Executing UpdateProductStock: franchiseId={}, branchName={}, productName={}, newStock={}",
                franchiseId, request.getBranchName(), request.getProductName(), request.getNewStock());

        return franchiseRepository.findById(franchiseId)
                .switchIfEmpty(Mono.error(new NotFoundException(
                        "Franquicia no encontrada con ID: " + franchiseId)))
                .flatMap(franchise -> validateAndUpdateProductStock(franchise, request))
                .doOnSuccess(response -> log.info("Product stock updated successfully: {} -> stock: {}",
                        request.getProductName(), request.getNewStock()))
                .doOnError(error -> log.error("Error updating product stock: {}", error.getMessage()));
    }

    private Mono<ProductResponse> validateAndUpdateProductStock(Franchise franchise, UpdateProductStockRequest request) {
        try {
            // Obtener la sucursal para validaciones adicionales
            Branch branch = findBranchByName(franchise, request.getBranchName());
            Product product = findProductByName(branch, request.getProductName());

            // Validación: el nuevo stock debe ser diferente al actual
            if (product.getStock() == request.getNewStock()) {
                return Mono.error(new InvalidOperationException(
                        String.format("El nuevo stock (%d) es igual al stock actual del producto",
                                request.getNewStock())));
            }

            // Delegar al agregado (validaciones automáticas del dominio)
            franchise.updateProductStock(
            ...
            Product updatedProduct = findProductByName(branch, request.getProductName());
            ...
            // Validación del dominio: stock negativo
            if (e.getMessage().contains("stock no puede ser negativo")) {
                return Mono.error(new InvalidOperationException(
                        "El stock no puede ser negativo. Valor proporcionado: " + request.getNewStock()));
            }
